...
class TestActivationTracking:
    ...
    def test_activation_id_returned_on_execute(
    ...
    def test_activation_contains_metadata(
        self, api_client, auth_token, sample_function
    ):
    ...
    def test_list_activations(self, api_client, auth_token):
        ...
        response = api_client.get(
            "/api/v1/iceruns/activations", headers={"Authorization": auth_token}
        )
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_filter_activations_by_function(
        self, api_client, auth_token, sample_function
    ):
    ...
    def test_activation_includes_duration(
        self, api_client, auth_token, sample_function
    ):
