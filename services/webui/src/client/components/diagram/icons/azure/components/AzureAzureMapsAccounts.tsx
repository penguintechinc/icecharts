import type { SVGProps } from 'react';
import { memo } from 'react';
const SvgAzureAzureMapsAccounts = (props: SVGProps<SVGSVGElement>) => (
  <svg
    id="a597878d-75df-4c17-b48a-8881b353adeb"
    xmlns="http://www.w3.org/2000/svg"
    width={18}
    height={18}
    viewBox="0 0 18 18"
    {...props}
  >
    <defs>
      <linearGradient
        id="b708c95a-acf9-48dc-86e6-fd00deeb187d"
        x1={9}
        y1={17.5}
        x2={9}
        y2={0.5}
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0} stopColor="#198ab3" />
        <stop offset={0.442} stopColor="#28b7db" />
        <stop offset={0.775} stopColor="#32d4f5" />
      </linearGradient>
    </defs>
    <g>
      <circle cx={9} cy={9} r={8.5} fill="url(#b708c95a-acf9-48dc-86e6-fd00deeb187d)" />
      <g>
        <path
          id="aea28990-0f7a-41ea-b630-96207e88670d"
          d="M10.282,2.757,3.941,12.82,11.88,9.742Z"
          fill="#9cebff"
        />
        <path
          id="ab7613f7-985d-4b9c-a222-e520a3439041"
          d="M9,4.79l3.934,9.54L10.282,2.757Z"
          fill="#fff"
          opacity={0.7}
        />
        <path
          id="f5bebea7-9a6d-4d44-be76-be804ddf602b"
          d="M8.772,10.947l2.386-.925,1.776,4.308Z"
          fill="#32bedd"
        />
      </g>
    </g>
  </svg>
);
const Memo = memo(SvgAzureAzureMapsAccounts);
export default Memo;
