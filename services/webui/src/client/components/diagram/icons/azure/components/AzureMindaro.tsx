import type { SVGProps } from 'react';
import { memo } from 'react';
const SvgAzureMindaro = (props: SVGProps<SVGSVGElement>) => (
  <svg
    id="f3fd6249-104f-4be9-975d-cbeefcf4479a"
    xmlns="http://www.w3.org/2000/svg"
    width={18}
    height={18}
    viewBox="0 0 18 18"
    {...props}
  >
    <defs>
      <linearGradient
        id="e56cab92-60db-4789-9ec0-59f44d4da934"
        x1={13.8}
        y1={4.27}
        x2={13.8}
        y2={9.34}
        gradientTransform="matrix(1, 0, 0, -1, 0, 20)"
        gradientUnits="userSpaceOnUse"
      >
        <stop offset={0} stopColor="#0078d4" />
        <stop offset={0.16} stopColor="#1380da" />
        <stop offset={0.53} stopColor="#3c91e5" />
        <stop offset={0.82} stopColor="#559cec" />
        <stop offset={1} stopColor="#5ea0ef" />
      </linearGradient>
    </defs>
    <title>{'Icon-Artboard 1'}</title>
    <g>
      <polygon points="13.23 2.49 13.23 7.33 9.08 9.76 9.08 4.91 13.23 2.49" fill="#949494" />
      <polygon points="13.23 2.49 9.08 4.92 4.93 2.49 9.08 0.07 13.23 2.49" fill="#b3b3b3" />
      <polygon points="9.08 4.92 9.08 9.76 4.93 7.33 4.93 2.49 9.08 4.92" fill="#e6e6e6" />
      <polygon points="8.61 10.67 8.61 15.49 4.47 17.92 4.47 13.09 8.61 10.67" fill="#949494" />
      <polygon points="8.61 10.67 4.47 13.1 0.31 10.67 4.47 8.24 8.61 10.67" fill="#b3b3b3" />
      <polygon points="4.47 13.1 4.47 17.92 0.31 15.49 0.31 10.67 4.47 13.1" fill="#e6e6e6" />
      <polygon
        points="13.74 8.23 9.59 10.66 9.59 15.51 13.74 17.93 17.9 15.51 17.9 10.66 13.74 8.23"
        fill="#50e6ff"
      />
      <path
        id="fe5a0693-7ac1-41ed-8008-1d80e4daeb81"
        d="M16.75,15.74H10.89a.57.57,0,0,1-.56-.58h0V11.21a.51.51,0,0,1,.47-.55h5.91a.6.6,0,0,1,.56.59v3.9a.52.52,0,0,1-.43.59h-.09Z"
        fill="url(#e56cab92-60db-4789-9ec0-59f44d4da934)"
      />
      <g id="a973cc63-1b5a-44cb-848b-8b6365660e39">
        <path
          d="M12.84,14.78l-1.57-1.55a.18.18,0,0,1,0-.24l1.6-1.49a.18.18,0,0,1,.22,0l.14.14a.15.15,0,0,1,0,.22h0L12,13a.17.17,0,0,0,0,.23l1.21,1.24a.14.14,0,0,1,0,.21h0l-.1.1a.16.16,0,0,1-.22,0Z"
          fill="#f2f2f2"
        />
      </g>
      <g id="b56fecf7-ed4b-4c05-8b69-1204b7229a87">
        <path
          d="M14.72,14.78l1.56-1.55a.16.16,0,0,0,0-.24L14.64,11.5a.15.15,0,0,0-.22,0h0l-.11.12a.16.16,0,0,0,0,.22L15.55,13a.16.16,0,0,1,0,.24l-1.19,1.22a.14.14,0,0,0,0,.21h0l.09.09a.14.14,0,0,0,.2.07A.16.16,0,0,0,14.72,14.78Z"
          fill="#f2f2f2"
        />
      </g>
    </g>
  </svg>
);
const Memo = memo(SvgAzureMindaro);
export default Memo;
