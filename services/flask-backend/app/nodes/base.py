"""
Base node framework for IceCharts Flask trigger system.

This module provides the foundational classes and interfaces for building trigger
nodes that execute on the Flask API and publish jobs to Redis Streams for worker
processing. All trigger implementations must inherit from BaseTrigger and implement
the fire method.
...
class BaseNode(ABC):
    """
    Abstract base class for all IceCharts workflow nodes.
...
class BaseTrigger(BaseNode):
    """
    Abstract base class for IceCharts trigger nodes.

    Triggers run on the Flask API node and publish jobs to Redis Streams for
    workers to process. When a trigger fires, it creates a NodeResult, then
    publishes the output to Redis so workers can begin execution of the remaining
    workflow graph.

    Class Attributes:
        category: Must be set to "triggers" for all trigger implementations.

    Methods:
        fire: Async method that fires the trigger and returns result.
        publish_job: Publishes execution job to Redis Stream for workers.
    """
    ...
    async def fire(
        ...
        """
        Execute the trigger and produce output data.

        Trigger implementations should:
        1. Validate trigger_data against expected parameters
        2. Perform the trigger operation (e.g., listen to webhook, poll API)
        3. Return a NodeResult with trigger output data
        4. Call publish_job to enqueue the workflow for workers

        Args:
            context: Execution context containing config, variables, and logger.
            trigger_data: Trigger-specific data that initiates the trigger.

        Returns:
            NodeResult containing trigger output data or error.
        """
        pass

    async def execute(self, context: NodeContext, inputs: Dict[str, Any]) -> NodeResult:
        """
        Execute the trigger by calling fire with trigger data.

        This method adapts the BaseNode.execute interface to trigger execution.

        Args:
            context: Execution context containing config, variables, and logger.
            inputs: Dictionary containing trigger data.

        Returns:
            NodeResult from fire method.
        """
        return await self.fire(context, inputs)
    ...
    async def publish_job(
        ...
        """
        Publish a job to Redis Stream for worker processing.

        Creates a job entry in the Redis Stream containing the trigger output
        and remaining workflow graph to be executed by workers.

        Args:
            redis_client: Connected Redis client instance.
            execution_id: Unique identifier for this workflow execution.
            playbook_id: ID of the playbook being executed.
            trigger_output: Output data from the trigger (output port data).
            remaining_graph: Remaining workflow nodes to execute after trigger.
        ...
        Raises:
            ...
            ValueError: If execution_id or playbook_id is empty.
            TypeError: If trigger_output or remaining_graph are not serializable.
        """
        if not execution_id:
        ...
        try:
            job_data = {
                ...
                "trigger_output": json.dumps(trigger_output),
                "remaining_graph": json.dumps(remaining_graph),
                "published_at": datetime.utcnow().isoformat(),
            }

            stream_key = f"icestreams:jobs:{playbook_id}"
            result = redis_client.xadd(stream_key, job_data)

            return result is not None
        except json.JSONDecodeError as e:
            raise TypeError(f"Failed to serialize job data: {e}") from e
        except redis.RedisError as e:
            raise redis.RedisError(f"Failed to publish job to Redis: {e}") from e
