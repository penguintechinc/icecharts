...
class TestIceRunsInvokerInit:
    ...
    def test_init_with_mocked_deps(
        self, invoker_instance, mock_redis, mock_docker_client, mock_db
    ):
...
class TestProcessTask:
    ...
    def test_process_task_reads_fields(self, invoker_instance, sample_task_data):
        ...
        with patch.object(invoker_instance, "_load_package", return_value=b""):
            with patch.object(
                invoker_instance, "_extract_package", return_value="/tmp/test"
            ):
    ...
    def test_process_task_updates_status_running(
    ...
    def test_process_task_updates_status_completed_on_success(
        self, invoker_instance, sample_task_data
    ):
    ...
    def test_process_task_updates_status_failed_on_nonzero_exit(
        self, invoker_instance, sample_task_data
    ):
        ...
        with patch.object(MetricsRecorder, "record_execution_start") as mock_start:
            with patch.object(MetricsRecorder, "record_execution_end") as mock_end:
                with patch.object(MetricsRecorder, "record_execution_complete"):
                    with patch.object(
                        invoker_instance, "_load_package", return_value=b""
                    ):
                        with patch.object(
                            invoker_instance, "_extract_package", return_value="/tmp/t"
                        ):
    ...
    def test_process_task_publishes_redis_events(
        self, invoker_instance, sample_task_data
    ):
    ...
    def test_process_task_cleans_temp_dir(self, invoker_instance, sample_task_data):
        ...
        with patch.object(invoker_instance, "_load_package", return_value=b""):
            with patch.object(
                invoker_instance, "_extract_package", return_value="/tmp/testdir"
            ):
    ...
    def test_process_task_handles_missing_runtime(
        self, invoker_instance, sample_task_data
    ):
