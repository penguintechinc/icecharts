...
ACTIVE_EXECUTIONS = Gauge(
    "iceruns_active_executions", "Number of currently running executions", ["worker_id"]
)

# Metric 5: Task queue size gauge
QUEUE_SIZE = Gauge("iceruns_queue_size", "Number of tasks in execution queue")
...
class MetricsRecorder:
    ...
    def record_execution_complete(
        ...
        EXECUTIONS_TOTAL.labels(
            runtime=runtime, status=status, trigger_type=trigger_type
        ).inc()

        EXECUTION_DURATION.labels(runtime=runtime, status=status).observe(
            duration_seconds
        )
    ...
    def record_execution_error(runtime: str, error_type: str):
        ...
        EXECUTION_ERRORS.labels(runtime=runtime, error_type=error_type).inc()
