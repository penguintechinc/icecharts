...
const allIconNames = Object.keys(IconoirIcons).filter(
  key => !key.startsWith('_') &&
         key !== 'default' &&
         typeof IconoirIcons[key] === 'function'
);
 ...
 * To regenerate, run: node scripts/generate-iconoir-icons.js
 *
...
// Write files
const webUIPath = join(__dirname, '../services/webui/src/client/components/diagram/icons/iconoir');

try {
  const indexContent = generateIndexFile();
  const categoriesContent = generateCategoriesFile();

  writeFileSync(join(webUIPath, 'index.tsx'), indexContent, 'utf8');
  console.log('✅ Generated index.tsx');

  writeFileSync(join(webUIPath, 'categories.ts'), categoriesContent, 'utf8');
